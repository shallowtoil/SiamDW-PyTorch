...
# Details: generate circle score map for FC-based trackers
# Date: 2019.4.28
# ------------------------------------------------------------------------------
...
def weighted_binary_cross_entropy(input, target, weight):
    ...
    for batch_id in range(target.shape[0]):
        loss_all.append(F.binary_cross_entropy_with_logits(input[batch_id], target[batch_id],
                                                           weight[batch_id], reduction='sum'))
