...
parser.add_argument('--dataset', default='OTB2015', choices=['OTB2015', 'OTB2013', 'VOT2017', 'none'], help='dataset test')
...
def update(net, s_x, x_crops, target_pos, window, p):
...
def track(state, im):
    ...
    target_pos, new_scale = update(net, p.s_x, x_crops.cuda(), target_pos, window, p)
...
def track_video(model, video):
    ...
    for f, image_file in enumerate(image_files):
        ...
        if f == start_frame:  # init
            ...
            state = init(im, target_pos, target_sz, model)  # init tracker
            location = cxy_wh_2_rect(state['target_pos'], state['target_sz'])
            regions.append(1 if 'VOT' in args.dataset else gt[f])
        elif f > start_frame:  # tracking
            state = track(state, im)
            location = cxy_wh_2_rect(state['target_pos'], state['target_sz'])
