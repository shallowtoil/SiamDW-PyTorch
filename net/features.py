...
class ResNet22(nn.Module):
    ...
    def load_pretrained(self):
        ...
        for k, v in pretrained_dict.items():
            layer = 0
            if len(k.split('.')[0]) >= 6 and k.split('.')[0][:5] == 'layer':
                layer = int(k.split('.')[0][5:])
                if layer > len(self.s2p_flags): break
            # blocks above 2 in layer with s2p
            if layer and self.s2p_flags[layer-1]:
                index = int(k.split('.')[1]) + 1
                if index > 1:
                    k_new = '.'.join([k.split('.')[0], str(index)]) + '.' + '.'.join(k.split('.')[2:])
                    if k_new in model_dict:
                        load_dict.update({k_new: v})
                        print('replacing', k, 'with', k_new)
                elif k in model_dict:
                    load_dict.update({k: v})
                    print('keeping', k)
            # params in layer0 and layer with no s2p
            elif k in model_dict:
                load_dict.update({k: v})
                print('keeping', k)
